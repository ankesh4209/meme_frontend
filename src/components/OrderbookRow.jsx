...
const OrderbookRow = ({ price, size, total, type }) => {
  // Depth calculation (controls background bar width)
  const depth = Math.min((total / 10) * 100, 100);
  const bgColor =
    type === "ask" ? "rgba(246, 70, 93, 0.2)" : "rgba(14, 203, 129, 0.2)";
  ...
  return (
    <div className="flex justify-between px-3 py-[3px] sm:py-[2px] relative group hover:bg-white/5 transition-colors cursor-pointer">
      ...
      {/* Price Column */}
      <span
        className={`relative z-10 w-1/3 text-left font-semibold ${type === "ask" ? "text-[#F6465D]" : "text-[#0ECB81]"}`}
      >
      ...
      {/* Total (Sum) column with slightly lighter color for mobile readability */}
